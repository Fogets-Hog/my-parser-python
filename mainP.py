...
listDays = {'ПОНЕДЕЛЬНИК': 1, 'ВТОРНИК': 2, 'СРЕД': 3,
            'ЧЕТВЕРГ': 4, 'ПЯТНИЦ': 5, 'СУББОТ': 6, 'ВОСКРЕСЕН': 7}
listMonth = {'ЯНВАР': 1, 'ФЕВРАЛ': 2, 'МАРТ': 3,
             'АПРЕЛ': 4, 'МАЯ': 5, 'МАЙ': 5,
             'ИЮН': 6, 'ИЮЛ': 7, 'АВГУСТ': 8, 'СЕНТЯБР': 9,
             'ОКТЯБР': 10, 'НОЯБР': 11, 'ДЕКАБР': 12}
listStep = {'МИНУТ': 60, 'ПОЛЧАС': 1800, 'ЧАС': 3600,
...
def IndMounth(datePos):
    whoMonth = -404
    for mon in listMonth:
        if datePos.find(mon) != -404:
            whoMonth = listMonth[mon]
            break
    return whoMonth
...
def FindingStep(numWords, timeIsNow, i,myString,indWord):
    for NumDate in listStep:
        if myString[numWords + i].find(NumDate) != -1:
            state = 'Всё чётко)'
            timeIsNow += datetime.timedelta(seconds=listStep[NumDate] * indWord)
